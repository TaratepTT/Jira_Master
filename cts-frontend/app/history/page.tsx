    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 h-14 flex items-center gap-3">
          ...
          <h1 className="text-sm font-semibold text-slate-800">ประวัติ Report ทั้งหมด</h1>
        </div>
      </header>
      <main className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
        {loading ? (
          <div className="space-y-2">{[1,2,3,4,5].map(i => <div key={i} className="h-16 rounded-xl bg-slate-200 animate-pulse"/>)}</div>
        ) : !reports.length ? (
          <div className="text-center py-16">
            <p className="text-slate-400 mb-4">ยังไม่มี report</p>
            <Link href="/" className="text-sm text-blue-600 hover:underline">อัปโหลดไฟล์แรก</Link>
          </div>
        ) : (
          <ul className="divide-y divide-slate-100 rounded-2xl border border-slate-200 bg-white overflow-hidden">
            {reports.map(r => (
              <li key={r.id} className="flex items-center gap-3 px-5 py-4 hover:bg-slate-50 transition-colors">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-400">{r.totalTickets} tickets · {new Date(r.createdAt).toLocaleDateString('th-TH', {day:'numeric',month:'short',year:'numeric'})}</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => handleDelete(r.id)} disabled={deletingId === r.id}
                    className="rounded-lg border border-slate-200 p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors disabled:opacity-40">
